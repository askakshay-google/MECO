...
def wait_for_bob_run(run_area, branch, node_pattern, timeout_minutes, check_interval_seconds, status_updater=None, summary_updater=None, is_single_node_check=False):
    ...
    while True:
        ...
        try:
            ...
            result = subprocess.run(bob_info_cmd, check=False, shell=False, timeout=60, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8', errors='ignore', cwd=run_area)
            if result.returncode != 0:
                if status_updater: status_updater(f"WARN: {log_prefix}: 'bob info' fail. Retrying...")
        ...
        try:
            ...
            if summary_text != last_statuses_summary:
                if status_updater: status_updater(f"INFO: {log_prefix}: Status: {summary_text}")
                    ...
                    for line in f_base:
                        ...
                        if m:
                            ...
                            if not os.path.isabs(base_chipfinish_src):
                                run_dir = os.path.dirname(eco_work_dir_abs)
                                base_chipfinish_src = os.path.abspath(os.path.join(run_dir, base_chipfinish_src))
                            chipfinish_source_dir = base_chipfinish_src
                            found_chipfinish = True
                            if status_updater: status_updater(f"DEBUG: Found chipfinish.source in base var: {base_chipfinish_src}")
                            break
            if not found_chipfinish:
                 raise ValueError(f"CRITICAL: 'bbset chipfinish.source' not found in base var file: {base_var_path}")
        else:
    ...
    try:
        ...
        for i, current_iter_name in enumerate(all_iterations):
            ...
            if not final_var_file or not _add_analysis_bbsets_to_var_file(final_var_file, current_iter_name, analysis_sequences_name, status_updater):
                raise RuntimeError(f"FAIL: Prepare var file for '{current_iter_name}'. See log for details (e.g., missing 'chipfinish.source' in base_var_file.var).")
            ...
            while stage_idx < len(stages_to_run):
                ...
                wait_status, all_statuses, wait_msg = wait_for_bob_run(eco_work_dir, current_iter_name, run_node_pattern, timeout_minutes, check_interval, status_updater)
                    ...
                    while True: # Key Node Failure Loop
                        ...
                        if new_block_specific_path != block_specific_var_path_for_run:
                            ...
                            if not new_var_file or not _add_analysis_bbsets_to_var_file(new_var_file, current_iter_name, analysis_sequences_name, status_updater):
                                status_updater("ERROR: Failed to recreate var file. Please try again."); continue

                            for s_idx in range(stage_idx, len(stages_to_run)):
                                stage_dir_path = os.path.join(eco_work_dir, current_iter_name, stages_to_run[s_idx])
                                if not os.path.isdir(stage_dir_path): continue
                                for node_d in os.listdir(stage_dir_path):
                                    target_var_path = os.path.join(stage_dir_path, node_d, "vars", "flow.var")
                                    if os.path.isfile(target_var_path):
                                        try: shutil.copy(new_var_file, target_var_path)
                                        except Exception as e: status_updater(f"ERROR: Failed updating {target_var_path}: {e}")

                        failed_info["is_failed_state"] = False
                        inval_cmd = ["bob", "update","status", "-r", eco_work_dir,"--branch", current_iter_name, "--invalidate", failed_key_node,"-f"]
                        rerun_cmd = ["bob", "run", "-r", eco_work_dir, "--branch", current_iter_name, "--node", failed_key_node]

                        if not run_bob_command(inval_cmd, work_dir=eco_work_dir, status_updater=status_updater) or \
                           not run_bob_command(rerun_cmd, work_dir=eco_work_dir, status_updater=status_updater):
                            status_updater(f"ERROR: Failed to rerun '{failed_key_node}'. Try again."); continue

                        retry_status, _, _ = wait_for_bob_run(eco_work_dir, current_iter_name, failed_key_node, timeout_minutes, check_interval, status_updater, is_single_node_check=True)
                        if retry_status == "VALID":
                            status_updater(f"INFO: Retry SUCCESS for '{failed_key_node}'. Resuming stage check.")
                            break
                        else:
                             status_updater(f"ERROR: Retry for '{failed_key_node}' also failed. Try again.")
                    continue
                else:
        ...
        err_msg += f"ERROR @ Br: {current_branch_for_error}, St: {current_stage_for_error}\nDETAILS: {e}\n"
...
class EcoRunnerApp(tk.Tk):
    def __init__(self):
        ...
        ttk.Label(config_frame, text="IP Name:", foreground="darkgreen").grid(row=current_row, column=0, padx=5, pady=3, sticky=tk.W); self.ip_var = tk.StringVar(value=DEFAULT_IP_NAME); self.ip_entry = ttk.Entry(config_frame, textvariable=self.ip_var, width=30); self.ip_entry.grid(row=current_row, column=1, padx=5, pady=3, sticky=tk.W); current_row+=1
        ttk.Label(config_frame, text="Chip Name:", foreground="darkgreen").grid(row=current_row, column=0, padx=5, pady=3, sticky=tk.W); self.chip_var = tk.StringVar(value=DEFAULT_CHIP_NAME); self.chip_entry = ttk.Entry(config_frame, textvariable=self.chip_var, width=30); self.chip_entry.grid(row=current_row, column=1, padx=5, pady=3, sticky=tk.W); current_row+=1
        ...
        ttk.Label(config_frame, text="Analysis per Iteration :", foreground="darkblue").grid(row=current_row, column=0, padx=5, pady=3, sticky=tk.W); self.analysis_input_text = tk.Text(config_frame, width=50, height=3, wrap=tk.WORD); self.analysis_input_text.grid(row=current_row, column=1, columnspan=2, padx=5, pady=3, sticky=tk.EW); self.analysis_input_text.insert(tk.END, DEFAULT_ANALYSIS_INPUT); current_row+=1
    ...
    def toggle_log_display(self):
        if self.log_toggle_var.get():
            if not self.status_frame.winfo_viewable():
                self.status_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0,25)); self.geometry("")
        else:
    ...
    def set_controls_state(self, mode):
        ...
        for widget in [self.start_button, self.clear_button, self.load_config_button, self.save_config_button,
                       ...
                       self.process_entry, self.eco_work_dir_name_entry, self.block_entry,
                       self.analysis_input_text]:
    ...
    def processing_complete(self, outcome):
        ...
        if outcome == "SUCCESS": messagebox.showinfo("Success", "ECO process finished successfully.")
        elif outcome == "FAILED": messagebox.showwarning("Failed", f"Run failed @ key node: {failed_info.get('specific_node','N/A')}\nFix issue (check {self.log_file_path}) and click CONTINUE, or ABORT.")
        else: messagebox.showerror("Error", f"Process ended with status: {outcome}\nCheck {self.log_file_path} for details.")
    ...
    def start_processing(self):
        ...
        try:
            ...
            self.log_file_path = os.path.join(eco_run_dir, f"{run_name}.log")
            if not os.path.exists(eco_run_dir): os.makedirs(eco_run_dir)
            if os.path.exists(self.log_file_path): os.remove(self.log_file_path)
            ...
            eco_logic_args = (
                ...
                eco_run_dir, DEFAULT_BOB_RUN_TIMEOUT_MINUTES, DEFAULT_BOB_CHECK_INTERVAL_SECONDS,
    ...
    def quit_app(self):
        if self.processing_thread and self.processing_thread.is_alive():
            if messagebox.askyesno("Confirm Quit", "A process is running. Quit anyway?"):
